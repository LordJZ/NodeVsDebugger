...
namespace NodeVsDebugger
{
    ...
    [ComVisible(true)]
    [Guid(Constants.DebugEngineGuid)]
    public class AD7Engine : IDebugEngine2, IDebugEngineLaunch2, IDebugProgram3, IDebugEngineProgram2, IDebugSymbolSettings100
    {
        ...
        private List<AD7PendingBreakpoint> m_pendingBreakpoints = new List<AD7PendingBreakpoint>();

        // Called from the engine's detach method to remove the debugger's breakpoint instructions.
        public void ClearBoundBreakpoints()
        {
            foreach (var pendingBreakpoint in m_pendingBreakpoints)
        ...
        {
            guidEngine = new Guid(Constants.DebuggerGuid);
            return Constants.S_OK;
        }
        ...
        int IDebugEngineLaunch2.CanTerminateProcess(IDebugProcess2 process)
        {
            ...
            try {
                ...
                return processId == m_debuggedProcess.Id ? Constants.S_OK : Constants.S_FALSE;
            } catch (Exception e) {
                return EngineUtils.UnexpectedException(e);
            }
        }
        ...
        {
            //Debug.Assert(Worker.MainThreadId == Worker.CurrentThreadId);
            m_pollThread.RunOperation(() => m_debuggedProcess.Break());
            return Constants.S_OK;
        }
        ...
        public int Continue(IDebugThread2 pThread)
        {
            ...
            m_pollThread.RunOperation(() => m_debuggedProcess.Continue(thread.GetDebuggedThread()));
            return Constants.S_OK;
        }
        ...
        public int Detach()
        {
            ...
            m_pollThread.RunOperation(() => m_debuggedProcess.Detach());
            return Constants.S_OK;
        }
        ...
        public int GetEngineInfo(out string engineName, out Guid engineGuid)
        {
            ...
            engineGuid = new Guid(Constants.DebuggerGuid);
            return Constants.S_OK;
        }
        ...
        public int ExecuteOnThread(IDebugThread2 pThread)
        {
            //Debug.Assert(Worker.MainThreadId == Worker.CurrentThreadId);
            var thread = (AD7Thread)pThread;
            m_pollThread.RunOperation(() => m_debuggedProcess.Execute(thread.GetDebuggedThread()));
            return Constants.S_OK;
        }
